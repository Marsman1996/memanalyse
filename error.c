...
void error_output(){
    ...
    for(i = 0; i < error_num; i++){
        ...
        fprintf(error_file, "0x%08x ", error_list[i].app_addr); 
        fprintf(error_file, "0x%08x ", error_list[i].real_esp); 
        fprintf(error_file, "0x%08x ", error_list[i].real_ebp);
        fprintf(error_file, "%u ", error_list[i].write);         
        fprintf(error_file, "%u\n", error_list[i].size); 
    }
}